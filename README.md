...
The six CSVs under `data/processed/` are committed project inputs for the future SQLite database builder. Including them allows a clean clone to support the quick-start database workflow without requiring the raw source files. The generated `data/campscout.db` database and its sidecar files remain local and are excluded from Git.
...
No database server, database user, password, host, port, or mandatory environment file is required. Python's standard-library `sqlite3` module will be used. The future database builder and application will default to the repository-relative path `data/campscout.db`; an optional `CAMPSCOUT_DB_PATH` environment variable may override it.
...
National-park cleaning and distance generation are implemented. The SQLite database builder, application query layer, and Streamlit application are not implemented yet.
...
```powershell
# NOT IMPLEMENTED: build the SQLite database from the six processed CSVs
python -m db.build
...
Do not infer implementation details from these placeholder command names. They may change when the relevant component is designed. In particular, the repository does not yet contain SQLite DDL, a database loader, an application query layer, or a Streamlit page.
