...
## Repository distribution and reproducibility

Raw source CSVs remain excluded from Git. Anyone running the complete ETL must obtain the three original files listed above and preserve their exact filenames and contents.

The six deterministic outputs under `data/processed/` are included in the repository to support the quick-start workflow and allow a clean clone to build the future SQLite database without the raw sources. They must never be edited manually and may only be regenerated by the ETL pipeline. The future generated database, `data/campscout.db`, and its SQLite sidecar files remain local and are not distributable repository inputs.
