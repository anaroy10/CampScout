...
**Status:** Accepted; database implementation is pending
...
The future database file is `data/campscout.db`. It will be generated from the six processed CSVs in `data/processed/`; the generated database and its sidecar files remain outside Git. Raw CSVs also remain outside Git. The processed CSVs are included in the repository so a clean clone can reproduce the future database-build and quick-start workflow without access to the raw sources. Running the full ETL still requires obtaining those original source files, and processed CSVs may only be regenerated by that pipeline.

SQLite's serverless architecture does not weaken the relational requirements. The future implementation must use primary keys, foreign keys, integrity constraints, indexes, transactions, parameterized SQL, and database validation. Foreign-key enforcement must be enabled on every SQLite connection. No schema, database builder, query layer, or Streamlit application is implemented by this decision-only migration.
...
2. Should the optional many-campgrounds-to-one-Recreation-Area link remain a nullable field when the SQLite schema is designed, or be represented by an audited relationship table?
3. Which reported duplicate candidates represent distinct facilities, and which represent source duplicates?
4. Should the materialized CSV distance matrix be loaded as a SQLite table, or should a future database implementation recalculate distance after an indexed prefilter?
